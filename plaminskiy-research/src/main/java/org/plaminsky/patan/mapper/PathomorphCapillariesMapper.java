...
public class PathomorphCapillariesMapper {
    ...
    public List<Kidneys> processPathomorphCapillaries(Sheet sheet) {
        List<Kidneys> pathomorphCapillariesList = new ArrayList<>();

        for (int i = 1; i <= sheet.getLastRowNum(); i++) {
            Row row = sheet.getRow(i);
            if (row != null) {
                Kidneys pathomorphCapillaries = new Kidneys();
